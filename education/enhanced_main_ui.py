...
class EnhancedEducationUI:
    ...
    def __init__(self, system_core, db_manager):
        ...
        logger.info("UI初始化完成（含知识图谱）")
    
    def create_interface(self) -> gr.Blocks:
        
        with gr.Blocks(title="智能教育系统", theme=gr.themes.Soft()) as interface:
    ...
    def _create_assessment_tab(self):
        ...
        next_btn.click(
            ...
            outputs=[
                ...
                quiz_area, report_area, report_display
            ]
        )
        ...
        restart_btn.click(
            ...
            outputs=[
                session_state, quiz_area, report_area,
                answer_input, submit_btn, next_btn, feedback_box
            ]
        )
...
"""
    ...
    def _next_question(self, session):
        """下一题 - 修复版:切换题目时隐藏上一题反馈"""
        if not session:
            return (
                None, "", "进度: 0/0", gr.update(visible=False),
        ...
        if session['current_index'] >= session['total_questions']:
            ...
            return (
                session, "", f"### 完成: {session['current_index']}/{session['total_questions']}",
        ...
        return (
            session, question['问题'], progress,
            gr.update(visible=False),  # 隐藏反馈框 - 关键修复
            gr.update(visible=True),
